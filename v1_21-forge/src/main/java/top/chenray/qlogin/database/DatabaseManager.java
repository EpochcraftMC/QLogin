...
public class DatabaseManager {
    ...
    public boolean unregisterPlayer(UUID uuid) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            if (deleted > 0) {
                LOGGER.info("玩家 {} 已被注销", uuid);
                return true;
            }
        } catch (SQLException e) {
            LOGGER.error("注销玩家失败", e);
        }
        return false;
    }
    ...
    public List<String> getAllPlayerNames() {
        ...
        String sql = "SELECT username FROM players ORDER BY username";
        try (Statement stmt = connection.createStatement()) {
    ...
    public Map<String, String> getPlayerInfo(UUID uuid) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                info.put("register_time", new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                    .format(new java.util.Date(rs.getLong("register_time"))));
                long lastLogin = rs.getLong("last_login");
                info.put("last_login", lastLogin > 0
                    ? new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date(lastLogin))
                    : "N/A");
                info.put("login_fail_count", String.valueOf(rs.getInt("login_fail_count")));
